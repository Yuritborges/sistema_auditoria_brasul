...
#   powershell -ExecutionPolicy Bypass -File tools\tag_release.ps1 -Versao 1.0.1 -Mensagem "Ajuste pendencias PDF"
...
if ($Versao -notmatch '^\d+\.\d+\.\d+$') {
    throw "Versao invalida: use formato X.Y.Z (ex.: 1.0.1)"
}
...
if ($Mensagem) {
    ...
    $msg = "Release $tag - Sistema Auditoria Brasul"
}
