...
class DashboardWidget(QWidget):
    ...
    def _build(self):
        ...
        self.lbl_mes = self._kpi("Total gasto no mês", "R$ 0,00", "R$")
        self.lbl_ano = self._kpi("Total gasto no ano", "R$ 0,00", "Σ")
        self.lbl_pend = self._kpi("Pedidos pendentes", "0", "P")
        self.lbl_sem_pdf = self._kpi("Pedidos sem PDF", "0", "!")
        cards.addWidget(self.lbl_mes._card, 0, 0)
        cards.addWidget(self.lbl_ano._card, 0, 1)
        cards.addWidget(self.lbl_pend._card, 0, 2)
        cards.addWidget(self.lbl_sem_pdf._card, 0, 3)
    ...
    def _kpi(self, titulo, valor, icone):
        card = QFrame()
        card.setObjectName("kpiCard")
        row = QHBoxLayout(card)
        row.setContentsMargins(16, 14, 18, 14)
        row.setSpacing(14)
